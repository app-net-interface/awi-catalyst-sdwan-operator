...
In this demo we will present how developers and admins can setup application connectivity between different VPCs using Application WAN Interface (AWI) system.

In AWS Cloud provider we have two VPCs: ML Model training VPC (source) and ML Data Set VPC (destination). Similar setup we have in GCP.

We have already established connection between source and destination VPCs, but we set default access control to deny all traffic, we want to enable connectivity only to specific VMs for specific ports and protocols.

We need to access data from ML backend VMs in Training VPC to API server VMs in Data Set VPC for training ML models. Other VMs should not be able to connect.

In this demo we will present AWI UI and AWI CLI and how we can use it to achive our goals.

read -p ""
clear
read -p "In our demo we have 2 EKS kubernetes clusters: 'ml-training-cluster' and 'ml-dataset-cluster' running in two different VPCs"
# show VPCs here
pe "eksctl get clusters"
read -p ""
pe "eksctl get cluster ml-training-cluster"
read -p ""
pe "eksctl get cluster ml-dataset-cluster"
read -p ""
...
In ML app cluster we have AWI system running"
#ml-training-cluster>> kubectl get pods -n awi-system
...
pe "kubectl apply -f config/samples/internetworkdomain_vpc_mltraining_to_vpc_mldataset.yaml"
read -p ""
...
pe "kubectl apply -f config/samples/appconnection_mltraining_to_mldataset.yaml"
read -p ""
read -p "After successful connection status of created CRD should be updated"
