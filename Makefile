...
# my.domain/awi-bundle:$VERSION and my.domain/awi-catalog:$VERSION.
IMAGE_TAG_BASE ?= 229451923406.dkr.ecr.us-west-2.amazonaws.com/awi/awi
...
IMAGE_TAG ?= ${COMMIT}
IMG ?= $(IMAGE_TAG_BASE)-controller:${IMAGE_TAG}
# ENVTEST_K8S_VERSION refers to the version of kubebuilder assets to be downloaded by envtest binary.
ENVTEST_K8S_VERSION = 1.24.2

KIND_CLUSTER_NAME ?= awi
...
manifests: controller-gen ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	$(CONTROLLER_GEN) rbac:roleName=manager-role crd:allowDangerousTypes=true,ignoreUnexportedFields=true webhook paths="./..." output:crd:artifacts:config=config/crd/bases
...
vet: ## Run go vet against code.
	# disabling copy locks because we use proto generated structs in which we do copy a mutex, but it shouldn't matter from functional perspective
...
.PHONY: kind-load
kind-load: ## load image to kind cluster
	kind load docker-image --name ${KIND_CLUSTER_NAME} ${IMG}

.PHONY: kind-deploy
kind-deploy: docker-build kind-load deploy ## build image and deploy in kind cluster
